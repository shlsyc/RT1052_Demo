...
#include "lv_code_convert.h"

static
const uint16_t uni2gbk[] = {
...
)
{
	...
	if (src < 0x80) {	/* ASCII */
		...
		if (dir) {		/* OEMCP to unicode */
			p = gbk2uni;
			hi = sizeof(gbk2uni) / 4 - 1;
		} else {		/* Unicode to OEMCP */
			p = uni2gbk;
			hi = sizeof(uni2gbk) / 4 - 1;
		}
		li = 0;
		for (n = 16; n; n--) {
			i = li + (hi - li) / 2;
...
//        bytes[2] = 0;
//        bytes[3] = 0;
    	bytes[0] = ((letter_uni>>0)  & 0x3F) | 0x80;
        bytes[1] = ((letter_uni>>6)  & 0x1F) | 0xC0;
        bytes[2] = 0;
        bytes[3] = 0;
    }
...
//        bytes[3] = 0;
        bytes[0] = ((letter_uni>>0)  & 0x3F) | 0x80;
        bytes[1] = ((letter_uni>>6)  & 0x3F) | 0x80;
        bytes[2] = ((letter_uni>>12) & 0x0F) | 0xE0;
        bytes[3] = 0;
    }
...
//        bytes[3] = ((letter_uni>>0)  & 0x3F) | 0x80;
        bytes[0] = ((letter_uni>>0)  & 0x3F) | 0x80;
        bytes[1] = ((letter_uni>>6)  & 0x3F) | 0x80;
        bytes[2] = ((letter_uni>>12) & 0x3F) | 0x80;
        bytes[3] = ((letter_uni>>18) & 0x07) | 0xF0;
    }
...
{
	uint16_t unicode;
	unicode = lv_convert(gbk_text,1);
	return lv_uni2utf8(unicode);
}
...
void lv_gbk2utf8(uint8_t *uft8_text,const char * gbk_text)
{
	...
	while(*gbk_text != 0)
	{
		...
		if(code1 < 0x80)
		{
			uft8_text[index++] = (uint8_t)lv_gbk2utf8_content(code1);
		}
		else
		{
			code2 = *(++gbk_text);
			hz_unicode = lv_gbk2utf8_content(code1<<8|code2);
			uft8_text[index++] = hz_unicode >> 16;
			uft8_text[index++] = hz_unicode >> 8;
			uft8_text[index++] = hz_unicode;
		}
		gbk_text++;
	}
}

uint8_t dstutf8_text[1024] = {0,};
uint8_t * lv_gbk2utf8_macro(const char * gbk_text)
{
	...
	while(*gbk_text != 0)
	{
		...
		if(code1 < 0x80)
		{
			dstutf8_text[index++] = (uint8_t)lv_gbk2utf8_content(code1);
		}
		else
		{
			code2 = *(++gbk_text);
			hz_unicode = lv_gbk2utf8_content(code1<<8|code2);
			dstutf8_text[index++] = hz_unicode >> 16;
			dstutf8_text[index++] = hz_unicode >> 8;
			dstutf8_text[index++] = hz_unicode;
		}
		gbk_text++;
	}
	dstutf8_text[index++] = 0;
	return dstutf8_text;
}
